...
class Interpreter(InterpreterBase):
    ...
    def __call_func(self, call_ast):
        ...
        if call_ast.elem_type == InterpreterBase.FCALL_DEF:
            ...
            target_closure = self.__obj_func_call(call_ast)
            if target_closure == None:
                super().error(ErrorType.NAME_ERROR, f"Function {func_name} not found")
            if type(target_closure) != Closure or target_closure.type != Type.CLOSURE:
                print(f"target closure tyupe : {type(target_closure)}")
                super().error(ErrorType.TYPE_ERROR, f"Function {func_name} is changed to not-function type.")
        ...
        if call_ast.elem_type == InterpreterBase.MCALL_DEF:
            self.__handle_method_call(call_ast, new_env)
    ...
    def __assign(self, assign_ast):
        ...
        if Interpreter.DOT_OP in var_name:
            self.__assign_object(var_name, src_value_obj)
            return
    ...
    def __eval_name(self, name_ast):
        ...
        if Interpreter.DOT_OP in var_name:
            object_name = var_name.split(Interpreter.DOT_OP)[0]
            field_name = var_name.split(Interpreter.DOT_OP)[1]
            object_val = self.env.get(object_name)

            result =self.__eval_object_name(object_val, field_name)
            return(result)
    ...
    def __obj_func_call(self, call_ast):
        ...
        if object_value == None:
            super().error(
                ErrorType.NAME_ERROR, f"{object_name} is not type object"
            )
        if object_value.type() != Type.OBJECT:
            super().error(
                ErrorType.TYPE_ERROR, f"{object_name} is not type object"
            )

        field_name = call_ast.get("name")
        func_to_run = self.__eval_object_name(object_value, field_name)
        return(func_to_run.value())

    def __assign_object(self, name, value):
        object_name = name.split(Interpreter.DOT_OP)[1]
        field_name = name.split(Interpreter.DOT_OP)[0]
        object = self.env.get(field_name)
        new_object = object.value()

        #Error Check
        if object == None:
            super().error(ErrorType.NAME_ERROR, f"Variable {field_name} not found")
        if object.type() != Type.OBJECT:
            super().error(ErrorType.TYPE_ERROR, f"Variable {field_name} is not a type OBJECT")

        if object_name == "proto":
            #Error Check
            if(value.type() != Type.OBJECT) and (value.type() != Type.NIL):
                super().error(ErrorType.TYPE_ERROR, f"{field_name} is not of type Object")

            new_object.proto = value
            new_object.new_env.set(object_name, new_object.proto, True)
            return
        
        if field_name == InterpreterBase.THIS_DEF:
            self.env.get(field_name).value().new_env.set(object_name,value)
            return

        new_object.new_env.set(object_name, value, True)

    def __handle_method_call(self, call_ast, new_env):

        if self.env.get('this') == (call_ast.get('objref')):
            new_env['this'] = self.env.get('this')

        if self.env.get('this') == None:
            new_env['this'] = self.env.get(call_ast.get('objref'))
    ...
    def __eval_object_name(self, object_val, field_name):

        #Error Check
        if(object_val == None) or (object_val.type() == Type.NIL):
            super().error(ErrorType.NAME_ERROR, f"Field '{field_name}' is not defined")

        elif object_val.type() != Type.OBJECT:
            super().error(ErrorType.TYPE_ERROR, f"Value is not of Type Object")
        
        result = object_val.value().new_env.get(field_name)

        if result != None:
            return result
        
        return self.__eval_object_name(object_val.value().proto, field_name)